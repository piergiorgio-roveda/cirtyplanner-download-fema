#!/usr/bin/env python3
"""
Script 06a: Extract FEMA NFHL State GDB ZIP Files

This script processes NFHL State GDB ZIP files downloaded by script 05_download_nfhl_gdb.py:
1. Extracts ZIP files containing FEMA National Flood Hazard Layer (NFHL) State Geodatabases
2. Logs extraction results to database with detailed file tracking
3. Stops after extraction (no merging)

This script is specifically designed for NFHL State GDB files, which are organized by state.
Each ZIP file is extracted to a folder named after the product name.

Prerequisites:
    - Script 04_get_nfhl_data_state_gdb.py: Database with NFHL GDB metadata
    - Script 05_download_nfhl_gdb.py: Downloaded NFHL GDB ZIP files
...
Usage:
    ...
    python notebooks/06a_extract_nfhl_zip_files.py --config custom_config.json
    
    # Process specific states only:
    python notebooks/06a_extract_nfhl_zip_files.py --states 01,02,04
    
    # Resume interrupted processing:
    python notebooks/06a_extract_nfhl_zip_files.py --resume
...
def setup_logging(verbose=False):
    ...
    log_file = os.path.join(log_dir, 'nfhl_extraction_06a.log')
...
def setup_database(config):
    ...
    if not os.path.exists(db_path):
        raise FileNotFoundError(f"Database not found: {db_path}. Please run scripts 04_get_nfhl_data_state_gdb.py and 05_download_nfhl_gdb.py first.")
...
def create_processing_tables(conn):
    ...
    # Track ZIP extraction status with detailed file logging
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS nfhl_extraction_log (
    ...
    cursor.execute('CREATE INDEX IF NOT EXISTS idx_nfhl_extraction_product ON nfhl_extraction_log (product_name)')
    cursor.execute('CREATE INDEX IF NOT EXISTS idx_nfhl_extraction_success ON nfhl_extraction_log (extraction_success)')
    
    conn.commit()

def extract_zip_file(product_name, zip_path, extract_to, conn, logger):
    """Extract ZIP file and log each GDB file found."""
    try:
        ...
        with zipfile.ZipFile(zip_path, 'r') as zip_ref:
            ...
            # Find and log each GDB file/directory
            for filename in extracted_files:
                if filename.lower().endswith('.gdb/') or filename.lower().endswith('.gdb\\') or '.gdb/' in filename.lower() or '.gdb\\' in filename.lower():
                    # Get the relative path within the ZIP
                    extracted_path = filename
                    # Get just the file name
                    file_name = os.path.basename(filename.rstrip('/').rstrip('\\'))
                    
                    extracted_gdb_files.append({
                        'extracted_path': extracted_path,
                        'file_name': file_name
                    })
        
        # Log each GDB file to database
        cursor = conn.cursor()
        for gdb_file in extracted_gdb_files:
            cursor.execute('''
                INSERT INTO nfhl_extraction_log
                (product_name, zip_file_path, extracted_path, file_name, extraction_success)
                VALUES (?, ?, ?, ?, ?)
            ''', (product_name, zip_path, gdb_file['extracted_path'],
                  gdb_file['file_name'], True))
        
        # If no GDB files found, still log the successful extraction
        if not extracted_gdb_files:
            cursor.execute('''
                INSERT INTO nfhl_extraction_log
                (product_name, zip_file_path, extracted_path, file_name, extraction_success)
                VALUES (?, ?, ?, ?, ?)
        ...
        logger.info(f"  [SUCCESS] Extracted {len(extracted_files)} files, found {len(extracted_gdb_files)} GDB files/directories")
        
        return {
            'success': True,
            'extract_dir': extract_dir,
            'extracted_files': len(extracted_files),
            'gdb_files_found': len(extracted_gdb_files)
        }
        ...
        cursor.execute('''
            INSERT INTO nfhl_extraction_log
            (product_name, zip_file_path, extracted_path, file_name, extraction_success, error_message)
            VALUES (?, ?, ?, ?, ?, ?)
...
def get_unique_zip_files(config, conn, target_states=None):
    """Get unique NFHL GDB ZIP files using the specified query to avoid duplicates."""
    zip_files = []
...
def extract_all_zip_files(config, conn, target_states=None, logger=None):
    ...
    cursor = conn.cursor()
    cursor.execute('SELECT DISTINCT product_name FROM nfhl_extraction_log WHERE extraction_success = 1')
...
def generate_extraction_report(extraction_results, conn, logger=None):
    ...
    logger.info("\n" + "=" * 80)
    logger.info("NFHL GDB ZIP EXTRACTION REPORT")
    logger.info("=" * 80)
    ...
    # Database statistics
    cursor = conn.cursor()
    cursor.execute('SELECT COUNT(*) FROM nfhl_extraction_log WHERE extraction_success = 1')
    total_successful_files = cursor.fetchone()[0]
    
    cursor.execute('SELECT COUNT(DISTINCT product_name) FROM nfhl_extraction_log WHERE extraction_success = 1')
    total_products_extracted = cursor.fetchone()[0]
    
    logger.info(f"\n[DATABASE STATISTICS]:")
    logger.info(f"  Total products extracted: {total_products_extracted}")
    logger.info(f"  Total GDB files logged: {total_successful_files}")
    ...
    logger.info("\n" + "=" * 80)
    logger.info("NFHL GDB EXTRACTION COMPLETED - Files are ready for use")
    logger.info("=" * 80)
...
def clear_extraction_logs(conn, target_states=None, logger=None):
    ...
    if target_states:
        ...
        state_placeholders = ','.join(['?' for _ in target_states])
        cursor.execute(f'''
            DELETE FROM nfhl_extraction_log
        ...
        logger.info(f"Cleared NFHL extraction logs for states: {target_states}")
    else:
        # Clear all extraction logs
        cursor.execute('DELETE FROM nfhl_extraction_log')
        
        logger.info("Cleared all NFHL extraction logs")
    
    conn.commit()
    
    # Get counts after clearing
    cursor.execute('SELECT COUNT(*) FROM nfhl_extraction_log')
    extraction_count = cursor.fetchone()[0]
    
    logger.info(f"Remaining NFHL extraction logs: {extraction_count}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Extract FEMA NFHL State GDB ZIP files')
    ...
    try:
        ...
        logger.info("=== FEMA NFHL GDB ZIP File Extraction (Script 06a) ===")
        logger.info(f"Start time: {datetime.now()}")
        
        config = load_config(args.config)
        
        # Update config with NFHL-specific paths if not present
        if 'nfhl_extraction_base_path' not in config['processing']:
            config['processing']['nfhl_extraction_base_path'] = "E:\\FEMA_NFHL_EXTRACTED"
            logger.info(f"Using default NFHL extraction path: {config['processing']['nfhl_extraction_base_path']}")
        ...
        if args.force_rebuild:
            ...
            download_dir = config['download'].get('nfhl_base_path', "E:\\FEMA_NFHL_DOWNLOAD")
            
            # Safety check: Ensure we're not accidentally targeting the download directory
            if extraction_base == download_dir:
                logger.error(f"SAFETY ERROR: NFHL extraction directory same as download directory - ABORTING FORCE REBUILD")
                return
        ...
        logger.info("\n=== NFHL GDB ZIP File Extraction ===")
        
        # Use NFHL-specific extraction path
        if 'extraction_base_path' in config['processing']:
            original_extraction_path = config['processing']['extraction_base_path']
            config['processing']['extraction_base_path'] = config['processing'].get('nfhl_extraction_base_path', "E:\\FEMA_NFHL_EXTRACTED")
            logger.info(f"Using NFHL extraction path: {config['processing']['extraction_base_path']}")
        
        extraction_results = extract_all_zip_files(config, conn, target_states, logger)
        
        # Restore original extraction path if it was changed
        if 'original_extraction_path' in locals():
            config['processing']['extraction_base_path'] = original_extraction_path
        ...
        logger.info(f"\nNFHL GDB extraction completed at: {datetime.now()}")
        logger.info("The extracted NFHL GDB files are now ready for use")
